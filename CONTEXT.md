...
**Initial Order By**:
The conditionally mandatory non-empty `initialOrderBy` baseline for a sorting-capable Table Instance. When at least one column is sortable, its entries use Column Identity and admit only `asc` or `desc`. Each `columnId` is inferred as the exact union of sortable identities from the table's `columns` tuple, providing autocomplete and compile-time rejection of unknown or nonsortable columns. A valid non-empty persisted `orderBy` takes precedence during restoration; later prop changes do not overwrite user intent, and resetting sorting returns to this baseline. Active sorting ranges from one entry through every sortable column and never reaches zero. A table with no sortable Column Identity forbids `initialOrderBy` and installs no normal sorting state, persistence, command, or UI.
