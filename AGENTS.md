...
73. TanStack owns generic interaction infrastructure whenever its supported libraries cover the capability. Use `@tanstack/react-hotkeys` exclusively for shortcut matching, modifier normalization, held-key state, registration, and listener lifecycle; BrunoTable may only declare bindings and translate matched gestures into typed table commands. Use TanStack Pacer, through `@tanstack/react-pacer` at React boundaries, for debounce, throttle, scheduling, and rate control that Pacer supports. Do not add handwritten or parallel engines. After reproducing a clear upstream bug, stop and ask the user before implementing any workaround.
