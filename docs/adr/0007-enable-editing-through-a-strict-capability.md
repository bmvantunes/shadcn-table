...
`BrunoTableClient` and `BrunoTableServer` use a discriminated editing interface: `editable: true` requires the pure typed `getRowVersion` extractor and the input-modality-neutral `onSaveEdits` Save Operation, while false or omitted editing rejects both and every other edit-only prop. Column `isEditable` declarations identify potentially editable columns and remain the authority for exact cell eligibility.
