...
If current Drag Fill preflight rejects any target on release, the whole gesture applies nothing and removes its preview. One accessible table-scoped `Fill rejected` toast explains the first deterministic failure and a bounded count of additional failures. It exposes Close but no Retry or mutation action and creates no draft, transaction, history, save actor, Save Operation, or partial edit.
