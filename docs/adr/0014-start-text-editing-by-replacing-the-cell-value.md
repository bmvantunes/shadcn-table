...
The seed is raw editor text, not a parsed value. Parsing, blank policy, synchronous local validation, draft creation, Batch history, and an Immediate Save Operation still occur only at Cell Edit Commit. An invalid candidate therefore remains in the editor under the existing blocking-validation contract.

After a locally accepted Cell Edit Commit, Enter moves the Active Cell one logical body row down in the same column and Shift+Enter moves it one logical body row up. This navigation follows local commit acceptance; it does not wait for an Immediate Save Operation to settle. A rejected parse or validation keeps the editor and Active Cell in place. Movement reveals an off-screen destination through the normal vertical virtualizer, never wraps at the first or last logical row, and uses the existing safe focus reconciliation if the commit changes filtering, sorting, or row presence. An active Linear Cell Range is the explicit exception: Enter and Shift+Enter retain it and cycle its Active Cell forward or backward through currently editable cells along its one axis.
