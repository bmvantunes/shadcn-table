...
The toolbar is absent when no children are supplied and augments rather than replaces the global filter/sort rail, overlays, or editable safety footer. Rendering location does not determine state ownership: Grid Filters are persisted grid intent, Quick Filter is session-only grid intent, and application-owned Source Constraints remain outside persisted grid preferences.
