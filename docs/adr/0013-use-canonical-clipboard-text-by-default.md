...
- V1 exposes no Cut or destructive cell Clear/Delete capability and registers no `Ctrl/Cmd+X`, `Delete`, or `Backspace` mutation handler. A value changes only through an editor, explicit paste transaction, or repetition-only Drag Fill transaction.
