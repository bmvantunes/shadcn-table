...
The complete admitted Grid Filter collection has one conservative aggregate admission ledger: at most 16,384 root entries, 16,384 retained node occurrences, 16,384 retained scalar or `in`-member operands, and 1,048,576 UTF-16 code units of retained raw, canonical, normalized, semantic-signature, and active-filter-label text. The same ledger has one 16,384-call semantic-equivalence work allowance for each complete collection admission transaction (initial normalization or a replacement), and charges each custom comparison before invoking Value Type code, including calls made by candidates that are later rejected. That work allowance is monotonic within the admission currently in progress; it is not a Table Instance lifetime quota, and Clear, Reset, removal, replacement, and column reconciliation may create a new admitted collection with fresh work accounting after retaining only their compiled roots and indexes. A maximum nesting depth of 64, dense-array checks, active-path cycle checks, and bounded individual operand shape remain structural validity guards for unreadable or hostile values; they are not alternate complexity budgets and do not reset or replace the aggregate ledger. The untrusted root snapshot has a separate 16,384-own-entry materialization cap so a hostile sparse or accessor-backed array cannot force unbounded index sorting and copying. `BrunoTableClient` rejects an initial collection that exceeds an aggregate bound with a `TypeError` at construction rather than silently broadening the result; conservative restoration drops an invalid collection, and a command candidate that would exceed the remaining aggregate capacity is rejected without changing the committed filter state.

Admission compiles each accepted root once. It retains the exact sanitized node graph, its owning Column Identity, compiled operand semantics, a collision-free semantic signature when one can be proved, a bounded active-filter label, and its aggregate cost. The collection also retains its per-column root, label, and signature indexes. Clear, Reset, replacement, active-filter snapshots, and command metadata operate on those compiled roots and indexes; they do not rediscover Column Identities or descriptions by recursively scanning the raw filter AST. Custom Value Semantics without a collision-free identity are compared conservatively as changed, which may cause an extra query transition but never suppresses a real semantic change. No JSON serialization or estimated JSON length participates in admission or equality.

Continuous overlay edits carry the retained root handle when one existing same-column root changes, so the Pacer callback re-admits only that candidate against the retained sibling roots. Structural edits such as adding or removing expression conditions may still replace the whole column collection through the same aggregate admission seam. Active-filter review exposes each retained root as an individual removal target; Clear All remains the separate whole-Grid-Filter command.
