...
- Text matching normalizes through NFD. The default accent-insensitive policy removes combining marks, while `accentSensitive: true` retains them: a contains operand `"e"` matches `"é"`, but an operand `"é"` does not match `"e"`. Case sensitivity is applied after the same normalization step.
