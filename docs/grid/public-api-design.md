...
> = BrunoTableBaseProps<TRow, TColumns> &
  ...
  BrunoTableGroupingCapability<TColumns> & {
    ...
    externalFilters?: ExactSourceWhere<TViewport>;
    editable?: never;
    getRowVersion?: never;
    onSaveEdits?: never;
  };
...
- `quickFilterFields` is an optional explicit non-empty tuple of string-valued Query Fields. TypeScript enforces the field shape and non-empty tuple; the runtime applies a defensive maximum of 256 entries while snapshotting configuration. BrunoTable never infers it from visible columns or accepts Column Identities in its place. Omitting it means the table has no Quick Filter capability.
- Text matching normalizes through NFD. The default accent-insensitive policy removes combining marks from both the row value and operand, so contains operands `"e"` and `"é"` match either form symmetrically. With `accentSensitive: true`, combining marks are retained. Case sensitivity is applied after the same normalization step.
...
- `BrunoTableServer` accepts application-controlled External Filters through effect-view-server's authoritative exact `Where` contract, including valid source fields without visible columns, and `AND`-combines them with Quick Filter and Grid Filters. They are reactive but never persisted, counted, reviewed, reset, or cleared by BrunoTable. Client Tables reject `externalFilters` because their complete `clientSource` already defines the caller's working set.
...
- The exact opt-in is `rowSelection?: true` on `BrunoTableClient`. Its Row Selection Column has fixed private identity `COL_ID_BRUNO_TABLE_ROW_SELECTION`, remains a leading sticky utility gutter, and is excluded from public column tuples, Logical Column Order, navigation, clipboard, query compilation, and every preference slice. No identity constant or column customization crosses the public interface.
...
- External Filters define an application-controlled Server working-set condition before Grid Filters. They are not persisted, counted, reviewed, reset, or cleared as grid preferences.
- Toolbar placement alone changes neither ownership nor persistence.

`BrunoTableFilterControl` makes that choice explicit. `ownership="grid"` supplies only the stable typed replace, clear, reset, and clear-all Grid Filter commands; it exposes no state reader or subscription. `ownership="external"` supplies no grid capability and simply marks an application-owned Server control; the wrapper never copies, counts, persists, resets, or interprets that application state. `BrunoTableServer.externalFilters` carries the application state through the source-owned exact `Where` contract without duplicating schema semantics. Client Tables reject `externalFilters`, and examples must not present External Filters as Client-owned behavior.
...
The type must reject a compound group containing different Column Identities. Cross-column Grid Filter composition is always the implicit public-array `AND`; Quick Filter owns its separate OR-across-eligible-fields behavior. For input compatibility, initial and persisted arrays may repeat a Column Identity. Admission deterministically groups those entries in first-seen column order and wraps each repeated column's entries, in their original order and without flattening or deduplication, in one same-column `AND` expression. Runtime and persisted state thereafter own at most one expression per Column Identity.

The complete admitted Grid Filter collection has one conservative aggregate admission ledger: at most 16,384 public entries, 16,384 retained node occurrences including canonical `AND` wrappers, 16,384 retained scalar or `in`-member operands, and 1,048,576 UTF-16 code units of retained raw, canonical, normalized, semantic-signature, and active-filter-label text. The same ledger has one 16,384-call semantic-equivalence work allowance for each complete collection admission transaction (initial normalization or a replacement), and charges each custom comparison before invoking Value Type code, including calls made by candidates that are later rejected. That work allowance is monotonic within the admission currently in progress; it is not a Table Instance lifetime quota, and Clear, Reset, replacement, and column reconciliation may create a new admitted collection with fresh work accounting after retaining only their compiled expressions and indexes. A maximum nesting depth of 64, dense-array checks, active-path cycle checks, and bounded individual operand shape remain structural validity guards for unreadable or hostile values; they are not alternate complexity budgets and do not reset or replace the aggregate ledger. The untrusted public array snapshot has a separate 16,384-own-entry materialization cap so a hostile sparse or accessor-backed array cannot force unbounded index sorting and copying. Canonicalization consumes a real wrapper node, so an otherwise maximum-sized duplicate-column input can be conservatively rejected at the edge. `BrunoTableClient` rejects an initial collection that exceeds an aggregate bound with a `TypeError` at construction rather than silently broadening the result; conservative restoration drops an invalid collection, and a command candidate that would exceed the remaining aggregate capacity is rejected without changing the committed filter state.

Admission sanitizes each accepted public entry once, then canonicalizes duplicate same-column entries into one expression and retains only that expression's exact node graph, Column Identity, compiled operand semantics, collision-free semantic signature when one can be proved, bounded active-filter label, and aggregate cost. The collection retains one expression, filter, label, and complexity index entry per filtered column. Clear, Reset, replacement, active-filter snapshots, persistence, and command metadata operate on these compiled expressions and indexes; they do not rediscover Column Identities or descriptions by recursively scanning raw filter ASTs. Custom Value Semantics without a collision-free identity are compared conservatively as changed, which may cause an extra query transition but never suppresses a real semantic change. No JSON serialization or estimated JSON length participates in admission or equality.

Each open column overlay owns one complete atomic local expression draft. Continuous valid edits publish that complete expression after 150 ms; immediate valid choices, adding or removing conditions, Clear, and Reset also operate on the complete column expression. An invalid complete draft remains local and cancels pending publication, so it cannot erase another authored condition. There are no retained root handles, partial-root commands, root-keyed pending maps, or sibling-root revision rules. Active-filter review exposes one entry per filtered column and removing it clears that column's complete expression; its count is therefore filtered-column count plus Quick Filter when active. Clear All remains the separate whole-Grid-Filter command.

For example, `contains` must be rejected for a numeric column and `greaterThan` must be rejected for a nonnumeric column.

`inRange` is half-open in both variants: `filter < filterTo` and `filter <= value < filterTo`. Client filtering must install the compiled exact comparator instead of TanStack's inclusive `inNumberRange` helper. Runtime filters retain native typed operands; the Server Adapter changes only Column Identity to Query Field and lets effect-view-server own schema-aware transport encoding.
...
Quick Filter IME handling owns one composition-session token. Clear or external invalidation marks the active token before publication; every late input remains suppressed and the controlled draft stays equal to the committed runtime value through the matching `compositionend`. A rejected Clear restores the committed runtime value, never the rejected uncommitted draft.
