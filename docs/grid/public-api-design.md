...
Each top-level Grid Filter expression admits at most 1,024 total node occurrences, including its root, a maximum nesting depth of 64 after that root, and at most 4,096 values in each `in` operand. This runtime work budget bounds hostile recursive input that has escaped the typed boundary without imposing that per-expression limit on the number of expressions in the implicit-AND root array. The untrusted root snapshot has a separate 16,384-own-entry materialization cap so a hostile sparse or accessor-backed array cannot force unbounded index sorting and copying; ordinary root collections remain implicit AND expressions. `BrunoTableClient` rejects an `initialFilters` expression that exceeds any expression bound with a `TypeError` at construction rather than silently broadening the result; conservative restoration drops an expression that exceeds any bound or the root materialization cap.
