...
Ordinary `valueFormatter` and `cellRenderer` callbacks are raw-row-aware and receive `TRow`, so BrunoTable must never invoke them with a grouped summary or fabricated source row. Aggregate Cells use the compiled aggregate-result Value Type presentation by default. A participating column may override that presentation through typed `aggregateValueFormatter` and `aggregateCellRenderer` callbacks receiving the aggregate `value`, `aggFunc`, Column Identity, ordered group-key values, and exact `rowCount`, but no raw `TRow`. Their `value` type is derived from the column's Value Type and selected aggregate function, including result-domain changes such as `countDistinct` producing `bigint`.
