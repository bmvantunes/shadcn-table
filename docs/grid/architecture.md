...
Each typed filter overlay owns one complete ephemeral column-expression draft outside persisted filter state. Exact Value Type parsing of the complete draft precedes Pacer publication. Parse failure updates only that overlay's compact validation snapshot; the last committed expression and row pipeline remain untouched. Closing discards the candidate. Adding and removing conditions changes this same draft. This validation state never enters table-root React state, query compilation, or `onPersistChange`.

Filter admission owns one collection-wide resource ledger for public entries, retained AST nodes, retained operands, retained text, and custom Select semantic-equivalence calls. Exact and canonical Select indexes are compiled with the column; only an unavoidable generic equivalence call consumes the shared comparison allowance, and it is charged before invocation even when the candidate is later rejected. Candidate-local node, operand, text, and metadata cost rolls back with a rejected candidate, while consumed semantic-comparison work cannot reopen the comparison allowance for a later hostile entry in that same complete admission transaction. Admission groups valid entries by Column Identity in first-seen order and wraps duplicate same-column entries in one metered `AND` node. The retained state and its indexes contain exactly one compiled expression per filtered column. Retained complexity is recalculated for the resulting collection, so replacement, Clear, Reset, or column reconciliation is not a permanent Table Instance quota. Commands use these expressions, operand plans, and per-column indexes rather than rediscovering filter structure. Root handles, partial-root commands, root-keyed pending maps, and sibling-root revision coordination are forbidden.

Pacer sits only on continuous text and numeric publication of the complete column-expression draft. One editor owns one pending whole-expression candidate, never condition- or root-owned work. Discrete checkbox and valid operator commands bypass it but still publish the complete expression. Bulk Set Filter actions calculate one normalized include/exclude result and dispatch one command, so preference encoding and row-pipeline generation happen at most once regardless of facet cardinality.

Quick Filter IME state is a single session token with idle, composing, and invalidated states. Invalidation cancels Pacer work and marks the active token before a Clear or external command crosses the editor gate. While invalidated, late input events restore the runtime-owned committed text and publish nothing; the matching `compositionend` consumes the token without interpreting its payload. This is session ownership, not a draft revision framework.
...
Quick Filter fields are immutable table configuration, not reactive grid state. The optional explicit non-empty `quickFilterFields` tuple contains string-valued Query Fields rather than Column Identities; TypeScript enforces its field shape and non-empty tuple, and runtime snapshotting defensively caps it at 256 entries. The tuple is never inferred from current visibility, order, or headers. The row-pipeline Adapter compiles committed text to `OR(contains(field, text), ...)`, then combines that group with External Filters and Grid Filters through `AND`. Client and Server pipelines preserve the same semantics without routing the expression through TanStack's global-filter contract.
...
The notification actor aggregates rejected operations into one table-scoped persistent toast and owns explicit dismissal. XState is the brain for legal transitions, operation ownership, aggregation, and dialog/toast lifecycles. BrunoTable-owned TanStack Store state is the memory for drafts, Batch History Commands, conflicts, operation references, and per-cell presentation deadlines. CSS owns the border-tracer and flash animation; neither actor nor store emits frame-by-frame events.

XState actors remain private behind that store seam. A Grid Command lets the actor decide one legal transition and then publishes one coherent immutable store projection; React renderers never subscribe directly to actors or join independently observed actor and store snapshots. Cells, footer controls, dialogs, and notifications select only their smallest TanStack Store projection, so no render boundary can observe an actor transition before its corresponding sparse memory update or vice versa.

One Batch History Command retains reversible before-and-after sparse cell edit state for every cell affected by the user gesture, including Draft and Conflict evidence. Value-only pairs cannot undo Mine rebasing or Server discard because those actions may change the Base and conflict state without changing presentation. The command stack is bounded, contains no full row-store snapshot, and prunes a converged Cell Identity from past and future commands so replay cannot resurrect source-confirmed work. Non-empty undo or redo history remains edit-owned work and therefore blocks Edit Mode switching until redo, a new command, or Reset disposes of it.

The operation repository is bounded separately from Batch history. Pending, awaiting-source, and rejected operations retain immutable submitted evidence plus narrow reverse indexes only while reconciliation or notification needs it. Completed operations leave at most their bounded flash/notification presentation record and are removed when that lifecycle ends; the Table Instance never accumulates every historical Save Operation.
...
The footer is the one persistent full-width edit safety rail. Complete sparse changes, conflicts,
blocked records, validation details, and operation diagnostics mount only inside on-demand review
surfaces. Do not reserve a permanent side rail or bottom inspector that reduces the centre-column or
virtual-row viewport. Conflict and Reset reviews reuse read-only internal `BrunoTableClient`
presentation rather than building a second formatting system.

Live-by-default is a source-lifecycle rule across the runtime. A mounted surface that presents current source state owns a narrow subscription for as long as it is mounted: grid cells, counts, filter results, Client Set Filter facets, Reset Review rows, conflict rows, and `Server now` values all update from the active source generation. Closing an overlay or dialog releases subscriptions that exist only for that surface. Immutable edit bases, undo records, and submitted Save Change Sets remain snapshots because they are workflow evidence, not claims about current source state.

Set Filter faceting follows source ownership. A Client Table derives complete live facets locally from admitted resident rows under Quick Filter and every other Grid Filter while excluding its own column expression. A Server Table uses a separate narrow whole-result grouped subscription under the current Feed Route, External Filters, Quick Filter, and every other Grid Filter while excluding only its own expression; it never reads its sparse viewport cache. Boolean and Select Field Columns install this surface by default; Text, Number, BigInt, and BigDecimal columns require `enableSetFilter: true`. Values and counts retain native typed semantics rather than display strings. The live facet projection merges with compact include-or-exclude intent so explicitly included or excluded values missing from current results remain reversible at count zero; absent values with no explicit intent require no retained record. Select All and a user command selecting the final available value both remove the filter atomically, but source publications never trigger that normalization. Only the open overlay subscribes, and closing it releases its facet resources without replacing the primary viewport generation.

For Client facets, immutable resident-row lineage and changed-index evidence limit comparison to the facet column, other-filter columns, and Quick Filter fields touched by a publication. When those dependencies remain equivalent, the facet store preserves its existing snapshot without a complete resident scan or notification.

Empty Set Filter inclusion intent compiles to a Match-None Filter Expression that excludes both present and future values. This is a first-class row-pipeline semantic: Client uses an explicit false predicate, while Server requires View Server's source-native representation. Neither pipeline rewrites it to an exhaustive value list or a negation of the current live facet.
...
The Adapter allocates and owns the generation token before calling `viewport.replace`, then closes it over the exact sink. Source callback hints such as `keepRenderedRows` are not generation authority and never allow a source controller to bridge semantic row spaces.
...
An eligible groupable normalized Field Column may store typed `groupKeyValueFormatter`, `groupKeyCellClassName`, and `groupKeyCellRenderer` overrides. The grouped render plan supplies the exact field value, owning Column Identity, and exact row count without a raw `TRow` or sibling Group Key evidence. When absent, the compiled field Value Type supplies default Group Key Cell presentation. Exact ordered Group Key evidence belongs only at a table-bound seam that owns the actual Column tuple.

An aggregate-capable normalized Field Column may additionally store typed `aggregateValueFormatter`, `aggregateCellClassName`, and `aggregateCellRenderer` overrides. The grouped render plan supplies the aggregate result, aggregate function, owning Column Identity, and exact row count without a raw `TRow` or sibling Group Key evidence. When absent, the compiled aggregate-result Value Type supplies default presentation. Aggregate result typing belongs to the selected function and Value Type rather than being assumed equal to the source field type.
...
The Client aggregate executor is one BrunoTable-owned pass over the complete filtered resident result. `min`, `max`, and `countDistinct` derive from compiled presence, ordering, equivalence, and canonical identity. Exact `sum` and `avg` delegate only to a branded Value-Type Aggregate Algebra whose function references were snapshotted during normalization; every returned value is decoded through that same Value Type before publication. TanStack stock aggregation functions are not semantic authorities. The optional Effect entry point carries BigDecimal arithmetic closures, while root code and declarations remain Effect-free. See [ADR 0031](../adr/0031-own-exact-aggregate-arithmetic-in-column-value-semantics.md).

The Client row-pipeline coordinator installs one immutable projection epoch. Its staged configuration sanitizes the current Group By tuple, grouped sorting, layout, and Rows presentation before candidate derivation; one runtime commit then publishes that configuration with the matching raw or grouped row authority and presentation columns. Body consumers never combine requested intent or replacement Column Definitions with a prior installed epoch. The stable Runtime View and fine row/cell subscriptions survive value-only publications; only accepted structural grouping transitions advance the narrow projection subscription and viewport layout key.
...
The Row Selection Column is a private renderer utility column with fixed identity `COL_ID_BRUNO_TABLE_ROW_SELECTION`. It is a leading sticky control gutter, not a normalized consumer column or member of Logical Column Order. The viewport renderer accounts for its fixed width separately from user-column geometry, including while ordinary pinning is active or suspended. Header state subscribes only to the current-projection selection summary and each mounted row checkbox subscribes only to its Row Identity. The preference and query modules never admit the utility identity.
...
That hard reset applies when the Group By tuple changes and when the user commits grouped sorting or filtering. Passive aggregate-value, Rows-count, and membership movement within one unchanged tuple preserves a surviving active Group Row Identity. If passive membership removes it, navigation falls back to the same clamped display index rather than row zero. Neither passive path auto-reveals a live move; the next navigation command reveals its destination through the ordinary geometry seam.

The column-visibility control reads the all-normalized-columns registry so every dormant ordinary column remains discoverable. Ordinary visibility commands for inactive keys and nonparticipating columns mutate only the durable base visibility state; participating aggregate visibility additionally changes the grouped projection. Capability checks reject active-key and Rows hide commands. No grouped-only visibility store or reconciliation path exists.
...
V1 requires no pointer drag-and-drop or keyboard pickup/drop workflow. Add, remove, and scoped `Alt+ArrowLeft`/`Alt+ArrowRight` dispatch the typed grouping commands directly. Each command validates current eligibility and position, replaces the ordered Group By tuple once, emits one complete preference snapshot, and installs at most one complete Client projection epoch. Grouping controls subscribe only to the normalized eligible-column registry and ordered active tuple; live rows, aggregate values, scrolling, and selection do not rerender them.
