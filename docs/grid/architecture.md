...
The Client clipboard command resolves paste shape before value parsing. One cell may broadcast to the selected rectangle; a larger matrix must match the selected row and column counts exactly or infer exactly its own dimensions from one Active Cell. This imperative command performs no tiling, clipping, transposition, or partial target application. It resolves the complete identity-keyed destination, parses and validates every cell, and publishes at most one immutable edit transaction without lifting candidate matrices into React state.

Rejected paste commands publish one bounded immutable `Paste rejected` diagnostic to a table-scoped toast channel. The channel retains at most one paste diagnostic: a later rejection replaces it, an accepted paste clears it, and explicit dismissal clears it. It is not the save-operation notification actor and exposes no Retry command. Toast rendering subscribes only to this compact diagnostic snapshot; row publications, selection movement, and scrolling do not notify it.
