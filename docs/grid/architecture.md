...
An eligible groupable normalized Field Column may store typed `groupKeyValueFormatter` and `groupKeyCellRenderer` overrides. The grouped render plan supplies the exact field value, Column Identity, ordered group-key values, and exact row count without a raw `TRow`. When absent, the compiled field Value Type supplies default Group Key Cell presentation.

An aggregate-capable normalized Field Column may additionally store typed `aggregateValueFormatter` and `aggregateCellRenderer` overrides. The grouped render plan supplies the aggregate result, aggregate function, Column Identity, ordered group-key values, and exact row count without a raw `TRow`. When absent, the compiled aggregate-result Value Type supplies default presentation. Aggregate result typing belongs to the selected function and Value Type rather than being assumed equal to the source field type.
