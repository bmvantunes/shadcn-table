...
Client Set Filter faceting follows row-pipeline ownership. A Client Table derives complete live facets locally from admitted resident rows under Quick Filter and every other Grid Filter while excluding its own column expression. Boolean and Select Field Columns install this surface by default; Text, Number, BigInt, and BigDecimal columns require `enableSetFilter: true`. Its values and counts retain native typed semantics rather than display strings. The live facet projection merges with compact include-or-exclude intent so explicitly included or excluded values missing from current results remain reversible at count zero; absent values with no explicit intent require no retained record. Select All and a user command selecting the final available value both remove the filter atomically, but source publications never trigger that normalization. The overlay alone subscribes to its facet store and releases both row and filter subscriptions on close. Immutable resident-row lineage and changed-index evidence let that store compare only the facet column, other-filter columns, and Quick Filter fields touched by a publication; if those dependencies remain equivalent, it preserves the existing snapshot without a complete resident scan or notification. A later Server implementation must use a separate narrow whole-result subscription rather than reading its sparse viewport cache.
