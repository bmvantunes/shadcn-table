...
Filter admission owns one collection-wide resource ledger for roots, AST nodes, operands, retained text, and custom Select semantic-equivalence calls. Exact and canonical Select indexes are compiled with the column; only an unavoidable generic equivalence call consumes the shared comparison allowance, and it is charged before invocation even when the candidate is later rejected. Candidate-local metadata rolls back with the candidate, while consumed admission work cannot reopen the allowance for a later hostile root in that same complete admission transaction. Retained complexity is recalculated for the resulting collection, so a legitimate replacement, Clear, Reset, removal, or column reconciliation is not a permanent Table Instance quota. After admission, commands use the retained roots, operand plans, and per-column indexes rather than rediscovering filter structure.
