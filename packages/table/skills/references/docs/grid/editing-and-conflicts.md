...
- Promise resolution removes submitted Batch drafts, conflicts, validation, and history immediately and creates Accepted Overlays. Without a confirming update, overlays and the mode-specific reconciliation locks remain; an accepted Save Change Set is not retained as a re-saveable draft or reissued.
- After Batch rejection, unconverged drafts remain available for review and a later explicit fresh-preflight Save. Live canonical convergence removes the matching changes and history; edited-field divergence follows normal conflict detection.
- Accepted Overlays yield to live value convergence, a different Row Version, or authoritative row disappearance. They do not become conflicts or drafts merely because confirmation is delayed.
