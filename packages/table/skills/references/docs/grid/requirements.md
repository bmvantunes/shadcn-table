...
Live data that changes a current sort key may reorder rows without creating a user sorting command. It must never reset vertical scroll or transfer the Active Cell to a different Row Identity merely because that row inherited the old index. Follow `rowId + columnId` to the new position when known without forcing scroll. While a Client Cell Edit Session is active, a sort-key move keeps the edited row at the same visual Y-coordinate through immediate fixed-row-height scroll anchoring: the row enters its correct sorted index and surrounding rows move, while frame-coalesced geometry adjusts `scrollTop` by the corresponding offset delta. Do not freeze the row out of sort order, animate after it, or drive anchoring through React or XState. For an ordinary sparse Server row, a known authoritative move follows the Row Identity, pure window eviction retains its logical identity and coordinate, and only a conflicting authoritative key at that coordinate or an authoritative row-count boundary clears the Active Cell while grid-root focus remains. A Client Linear Cell Range follows its stable row identities only while they still form one contiguous range; otherwise clear it rather than inventing a disconnected selection.
...
Horizontal virtualization is equally mandatory. A table with 150 centre columns must not mount all 150 cells for every visible row merely because its rows are virtualized. One grid-level horizontal virtualizer windows the currently visible centre columns; pinned-start and pinned-end columns remain mounted in separate sticky regions outside that window while pinning is active and always participate in the same Logical Column Order. Before measurement, every layout with pinned columns suspends pinning. After measurement, suspension continues when a narrow mixed layout cannot reserve at least 80 CSS pixels for the centre or when a centreless pinned layout exceeds the viewport. While suspended, all columns are virtualized in start → centre → end order with zero pinned insets; widening the viewport until the active pinned layout fits restores the sticky regions automatically. Header and body consume the same immutable column-window snapshot so widths, virtual padding, hit testing, and keyboard reveal cannot drift.

Fixed-geometry keyboard reveal compares the destination bounds with the visible band after the active sticky header, pinned-start, and pinned-end insets, then applies only the smallest clamped scroll delta needed to expose the target. Do not use a virtualizer's nearest-index alignment for Cell navigation. A fully visible destination, or a pinned destination while pinning is active, causes no horizontal scroll. While pinning is suspended, formerly pinned destinations reveal inside the continuous all-column window with zero pinned insets. Initial Active Cell installation is one-shot and must not replay when a virtual window rerenders.
...
The shared lifecycle UI uses components from `@bruno/shadcn`. Every loading publication renders fixed-height `Skeleton` rows, even when that publication contains a complete candidate row array. Loading candidates never become coherent display evidence; the Client Adapter may retain only previously accepted coherent evidence privately so a later stale, closed, or error publication can reuse it. Stale results retain their coherent rows, including valid empty results, under a compact persistent warning `Alert` titled `Live data delayed`. Closed results retain rows under `Live updates stopped`, or show a full-body `Empty` state when none are available. Error results retain rows under a destructive `Live data error` Alert, or show a full-body destructive Empty state without rows. Supporting status codes and messages are bounded plain text. These states are source-authoritative and non-dismissible; recovery removes them only through a later source snapshot.
...
- `inRange` is half-open with strict bounds: `filter < filterTo` and `filter <= value < filterTo`;
...
Persisted column pinning is one complete durable base projection for every identity present in the persisted column order. Restoration commits start, end, and explicit unpinned intent even when it equals the current definition baseline, so a later column-definition replacement cannot erase that restored choice. A current column absent from the persisted order was introduced outside that snapshot and retains its current definition pinning.
...
Header and layout menus may group Sort, Filter, Pin start/end, Unpin, Move, Visibility, and Reset affordances with checked state and non-color cues. They dispatch the same typed Grid Commands as pointer and keyboard interactions; they never receive or expose a TanStack Column/Table object. Hiding remains rejected when it would remove the final navigable column, and no table offers an action that removes its final active sort.
...
Each Column Identity owns at most one committed Grid Filter Expression. Public, restored, or initial arrays may contain multiple entries for the same column for compatibility; admission preserves their order and canonicalizes them to one same-column `AND` expression. The column editor owns one complete atomic local draft and publishes only that complete expression. Invalid complete drafts remain local and publish nothing, so no condition can commit independently of or erase another authored condition. Active-filter count is the number of filtered columns, plus Quick Filter when active; removing a column entry clears that complete expression.

Every committed semantically effective Grid Filter or Quick Filter change creates one query generation, resets logical row position and vertical scroll to zero, preserves horizontal scroll and column layout, and reconciles the Active Cell against the new row-zero projection without retaining a hidden row identity. A committed raw Quick Filter text change whose normalized semantics are unchanged still updates its narrow session snapshot and position-reset epoch, but does not create a redundant row-model generation. When rows remain, body navigation starts at row zero and its first visible navigable column; an empty result clears the body Active Cell while header navigation remains available. DOM focus stays with the filter overlay, toolbar control, or header control that initiated the command. This command-specific reset is distinct from a passive live Client publication, which retains a surviving Active Cell by identity and uses the ordinary same-row-domain fallback.

This command reset intentionally disputes Codex P1 review discussion [#46](https://github.com/bmvantunes/shadcn-table/pull/46#discussion_r3791204520), which proposed retaining a surviving body `rowId + columnId` across a committed filter generation. That identity-first rule belongs to passive live publications; applying it to a filter command can preserve a non-zero or newly hidden body row after the required row-zero reset. A header Active Cell is retained because it is outside the body row position, preserves coherent header navigation, and does not retain a hidden row identity.
...
Quick Filter owns one composition-session token. Any command or external synchronization that invalidates an active composition marks that token invalid before attempting publication. Every later input event from that session is ignored and the controlled input is restored from the committed runtime value until the matching `compositionend` consumes the token. Rejection never restores the rejected uncommitted draft.
...
The opt-in prop is `rowSelection?: true` on `BrunoTableClient`; `BrunoTableServer` rejects it. The enabled Row Selection Column has the fixed private identity `COL_ID_BRUNO_TABLE_ROW_SELECTION` and remains the leading sticky control column independently of user start/end pinning or pinning suspension. It is not a consumer column, is excluded from Logical Column Order and Active Cell navigation, and never enters column order, visibility, widths, pinning, filtering, sorting, grouping, clipboard, or persisted preferences. Its fixed compact width is implementation-owned. This identity and column do not become public exports or consumer configuration.
...
Pinned-start, centre, and pinned-end columns form one Logical Column Order. One horizontal key command moves exactly one navigable column. While pinning is active, centre scrolling reveals that destination with the minimum delta after accounting for both pinned-region widths; while pinning is suspended, every destination reveals with the minimum delta inside the zero-inset all-column window.
