...
This section specifies the shipped issue #17 contract. It requires effect-view-server 4.2.8 or
...
The compatible effect-view-server 4.2.8 release exposes that source-owned semantic identity through
the Viewport Source's opaque `semanticKey` contract. [effect-view-server#481](https://github.com/bmvantunes/effect-view-server/pull/481)
exposes the dependency-free query authority witness so leased Feed Routes and exact application conditions do not fall back to
...
The public effect-view-server Viewport Source preserves TypeScript row/query types and owns one
authoritative `completeRawSelect` tuple, but exposes no duplicate field-semantics registry. Raw
columns therefore declare Value Type explicitly, while typed Column Helpers such as
...
effect-view-server 4.2.8 exposes source-owned, declaration-only invariant witnesses through
`LiveQueryViewportBaseRow`, `LiveQueryViewportRouteBy`, `LiveQueryViewportWhere`, and
`LiveQueryViewportQueryAuthority`. The
...
Live View Server publications may move a row when an active sort key changes. Such movement does not restart the generation or reset scroll. For ordinary raw sparse rows, if the Active Cell's Row Identity remains in the known sparse window, reconcile its absolute index without automatically scrolling after it. If ordinary window eviction merely unloads that identity, retain its logical identity and coordinate. Clear the raw-row Active Cell while retaining browser focus on the grid root only when the source publishes a conflicting authoritative key at that coordinate or an authoritative row-count boundary removes it; never transfer activation to the different raw row now occupying the old index. Unchanged grouped projections instead use the clamped previous-display-index fallback specified under Read-only interaction contract below when the active Group Row Identity is unresolved or removed.
