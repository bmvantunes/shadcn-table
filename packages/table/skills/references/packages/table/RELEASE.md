...
The version-2 artifact manifest distinguishes `baseRevision` (the starting Git commit) from
`sourceSnapshot.sha256` (the exact working-tree input, including staged, unstaged, and untracked
files and excluding deleted files). Uncommitted changes are intentionally supported for precommit
validation. `sourceSnapshot.fileList` names the retained per-file hashes; the retained snapshots
contain the validated bytes. The base revision alone does not identify those bytes.
