...
describe("persistent toast accessibility", () => {
  ...
  test("keeps timeout-enabled collapsed Close controls under Base UI ownership", async () => {
    ...
    const close = toast.element().querySelector<HTMLButtonElement>('[data-slot="toast-close"]');
  ...
  test("keeps a high-priority persistent Close control accessible with one alert announcement", async () => {
    ...
    await expect.element(screen.getByRole("button", { name: "Close toast" })).toBeInTheDocument();
    const root = viewport.element().querySelector<HTMLElement>('[data-slot="toast"]');
  ...
  test("announces an error title and explanation once without an alert duplicate", async () => {
    ...
    expect(failure.element().getAttribute("aria-labelledby")).toMatch(/.+/u);
    expect(failure.element().getAttribute("aria-describedby")).toMatch(/.+/u);
    ...
    const title = failure.element().querySelectorAll('[data-slot="toast-title"]');
    const description = failure.element().querySelectorAll('[data-slot="toast-description"]');
  ...
  test("keeps one accessible Close control for every persistent toast in collapsed and expanded stacks", async () => {
    ...
    expect(closeButtonElements).toHaveLength(3);
    for (const button of closeButtonElements) {
